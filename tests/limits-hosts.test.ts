...
const config = {
  ...
  limits: { maxWatchersTotal: 10, maxWatchersPerSkill: 5, maxConditionsPerWatcher: 10, maxIntervalMsFloor: 1000 }
};
...
describe('allowed host normalization', () => {
  it('allows canonical host and host:port', () => {
    ...
    expect(() => assertHostAllowed(config as any, 'https://api.github.com:443/events')).not.toThrow();
  });

  it('rejects non-allowlisted host', () => {
    expect(() => assertHostAllowed(config as any, 'https://evil.example.net')).toThrow('Host not allowed');
  });
});
